...
public class ChatGui : MonoBehaviour, IChatClientListener
{
	...
	private readonly Dictionary<string,FriendItem> playerListUIItem =  new Dictionary<string, FriendItem>();
	...
	public void OnConnected()
	{
        ...
        this.chatClient.PublishMessage("system", "sistemMesajı 'hi'.");
    ...
    public void OnCreateRoom()
    {
        if (this.ChannelsToJoinOnConnect != null && this.ChannelsToJoinOnConnect.Length > 0)
        {
            string[] tempChannelName = new string[this.ChannelsToJoinOnConnect.Length+1];
            
            for(int i = 0; i < this.ChannelsToJoinOnConnect.Length; i++)
            {
                tempChannelName[i] = this.ChannelsToJoinOnConnect[i];
            }
            this.ChannelsToJoinOnConnect = tempChannelName;
            this.ChannelsToJoinOnConnect[this.ChannelsToJoinOnConnect.Length-1] = "Room"+ (this.ChannelsToJoinOnConnect.Length - 1);
            this.chatClient.Subscribe(this.ChannelsToJoinOnConnect, this.HistoryLengthToFetch);
        }
    }
	...
	public void OnStatusUpdate(string user, int status, bool gotMessage, object message)
	{
		...
		if (playerListUIItem.ContainsKey(user))
		{
			...
			if ( _friendItem!=null) _friendItem.OnFriendStatusUpdate(status);
		}
	}
